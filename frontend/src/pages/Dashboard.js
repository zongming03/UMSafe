...
const Dashboard = () => {
  ...
  const updateChartData = (range) => {
    ...
    const buildChartData = (rangeLabel) => {
      ...
      if (rangeLabel === "Today") {
        ...
        for (let cur = new Date(s); cur.getTime() <= e.getTime(); cur.setDate(cur.getDate() + 1)) {
          ...
          labels.push(`${bs.getFullYear()}-${String(bs.getMonth() + 1).padStart(2, "0")}-${String(bs.getDate()).padStart(2, "0")}`);
        }
      }
      ...
      buckets.forEach(([bs, be]) => {
        ...
        const openCount = createdInBucket.filter((c) => normalizeStatus(c.status) === "Opened").length;
        const inProgCount = createdInBucket.filter((c) => normalizeStatus(c.status) === "InProgress").length;
        const resCount = createdInBucket.filter((c) => normalizeStatus(c.status) === "Resolved").length;
    ...
    chart.setOption({
      ...
      series: [
        { name: "Open", type: "bar", stack: "total", data: data.openSeries, itemStyle: { color: "#FCD34D" } },
        { name: "In Progress", type: "bar", stack: "total", data: data.inProgressSeries, itemStyle: { color: "#60A5FA" } },
        { name: "Resolved", type: "bar", stack: "total", data: data.resolvedSeries, itemStyle: { color: "#34D399" } },
      ],
  ...
  const navigateToComplaint = (complaint) => {
    if (!complaint) return;
    // navigate to complaint detail route and pass the complaint object in state
  ...
  const handleKanbanMove = async (complaintId, newStatus) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/admin/complaints/${complaintId}/status`, {
        method: "PATCH",
      ...
      if (data) {
        setComplaints((prev) => prev.map((c) => (c.id === complaintId ? { ...c, ...data } : c)));
      }
    } catch (err) {
      console.warn("Failed to persist kanban move, rolling back:", err);
      // rollback to previous status
      setComplaints((prev) => prev.map((c) => (c.id === complaintId ? { ...c, status: previousStatus || "Opened" } : c)));
    }
  };
  ...
  const matchesStatusFilter = (c, statusFilter) => {
    ...
    if (sf === "open" || sf === "opened") return raw === "open" || raw === "opened" || raw.includes("open");
    if (sf === "in progress" || sf === "inprogress" || sf === "in-progress") return raw.includes("inprogress") || raw.includes("in progress") || raw.includes("in-progress");
    if (sf === "resolved") return raw.includes("resolved") || raw.includes("resolve");
  ...
  const matchesAssignedFilter = (c, assignedFilter) => {
    ...
    if (assignedFilter === "Unassigned") {
      return !c.adminId || c.adminId === "Unassigned" || (c.assignedTo === "Unassigned");
    }
    ...
    return String(adminName).toLowerCase() === String(assignedFilter).toLowerCase();
  };
  ...
  const filteredComplaints = complaints.filter((c) => {
    // search
    if (q && !((c.title || "").toLowerCase().includes(q) || (c.description || "").toLowerCase().includes(q) || (c.id || "").toLowerCase().includes(q))) {
      return false;
    }
    // applied filters
  ...
  function getPeriodBounds(rangeLabel, now = new Date()) {
    ...
    switch (rangeLabel) {
      ...
      case "Last 7 Days": {
        const e = normalizeDayEnd(end);
        const s = normalizeDayStart(new Date(end.getTime() - (7 - 1) * 24 * 60 * 60 * 1000));
        const prevEnd = new Date(s.getTime() - 1);
        const prevStart = normalizeDayStart(new Date(prevEnd.getTime() - (7 - 1) * 24 * 60 * 60 * 1000));
        return { start: s, end: e, prevStart, prevEnd: normalizeDayEnd(prevEnd) };
      }
      case "Last 30 Days": {
        const e = normalizeDayEnd(end);
        const s = normalizeDayStart(new Date(end.getTime() - (30 - 1) * 24 * 60 * 60 * 1000));
        const prevEnd = new Date(s.getTime() - 1);
        const prevStart = normalizeDayStart(new Date(prevEnd.getTime() - (30 - 1) * 24 * 60 * 60 * 1000));
        return { start: s, end: e, prevStart, prevEnd: normalizeDayEnd(prevEnd) };
      }
      case "Last 3 Months": {
        ...
        return { start: startCal, end: e, prevStart: normalizeDayStart(prevStart), prevEnd: normalizeDayEnd(prevEnd) };
      }
      default:
  ...
  function computeChange(current, previous) {
    ...
    if (!previous) {
      ...
      return { percent: null, label: `New (${delta > 0 ? `+${delta}` : delta})`, delta };
    }
    const rawPct = (delta / previous) * 100;
    const pct = Math.round(rawPct * 10) / 10; // one decimal
    const sign = pct > 0 ? "+" : "";
    return { percent: pct, label: `${sign}${pct}% (${delta > 0 ? `+${delta}` : delta})`, delta };
  }

  // compute counts for selectedTimeRange
  const { start: periodStart, end: periodEnd, prevStart, prevEnd } = getPeriodBounds(selectedTimeRange);
  const totalCurrentPeriod = countCreatedInRange(complaints, periodStart, periodEnd);
  ...
  const resolvedCurrentPeriod = countResolvedInRange(complaints, periodStart, periodEnd);
  const resolvedPrevPeriod = countResolvedInRange(complaints, prevStart, prevEnd);
  const resolvedChange = computeChange(resolvedCurrentPeriod, resolvedPrevPeriod);
  ...
  const normalizeStatus = (raw) => {
    ...
    if (s === "resolved" || s === "resolve" || s === "closed") return "Resolved";
  ...
  const handleFilterReset = () => {
    const empty = { status: "", priority: "", dateFrom: "", dateTo: "", assignedTo: "" };
  ...
  useEffect(() => {
    ...
    const option = {
      ...
      yAxis: { type: "value" },
      series: [
        { name: "Open", type: "bar", stack: "total", data: [], itemStyle: { color: "#FCD34D" } },
        { name: "In Progress", type: "bar", stack: "total", data: [], itemStyle: { color: "#60A5FA" } },
        { name: "Resolved", type: "bar", stack: "total", data: [], itemStyle: { color: "#34D399" } },
      ],
    };
  ...
  useEffect(() => {
    const loadAdmins = async () => {
      try {
        const res = await fetch("http://localhost:5000/admin/usersMobile/users", { credentials: "include" });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        ...
        admins.forEach((a) => {
          ...
          const name = a.name || a.username || `${a.firstName || ""} ${a.lastName || ""}`.trim() || a.displayName;
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="relative">
                      ...
                      {isTimeRangeOpen && (
                        <div id="timeRangeDropdown" className="time-range-dropdown">
                          <div className="py-1">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        ...
                        className={`px-3 py-1 rounded text-sm border ${
                          viewMode === "kanban" ? "bg-blue-50 text-blue-600" : "bg-white text-gray-600"
                        }`}
                      >
                        Kanban
                      </button>
                    ...
                    {getCurrentPageComplaints().map((complaint) => (
                      <tr
                        ...
                        {(() => {
                          ...
                          const MAX = 50;
                          const display = subj.length > MAX ? `${subj.slice(0, MAX - 1)}…` : subj;
                          return (
                            <>
                              <td className="complaint-table-row-subject">
                                <span className="inline-block max-w-xs truncate" title={subj}>
                                  {display}
                                </span>
                              </td>
                              <td className="complaint-table-row-status">
                                {(() => {
                                  const statusLabel = normalizeStatus(complaint.status);
                                  return (
                                    <span
                        ...
                        <td className="complaint-table-row-date">
                          {formatDateShort(complaint.createdAt || complaint.updatedAt || complaint.date)}
                        </td>
                        <td className="complaint-table-row-status">
                          {(() => {
                            const priorityLabel =
                              complaint.priority || complaint.category?.priority || "";
                            return (
                              <span className={`font-medium ${getPriorityColor(priorityLabel)}`}>
                                {priorityLabel}
                              </span>
                            );
                          })()}
                        ...
                        <td className="complaint-table-row-assigned">
                          {adminsMap[complaint.adminId] || complaint.adminName || complaint.assignedTo || "Unassigned"}
                        </td>
