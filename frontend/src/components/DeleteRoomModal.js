...
const DeleteRoomModal = ({
  ...
  const rooms = Array.isArray(roomToDelete)
    ? roomToDelete
    : roomToDelete
    ? [roomToDelete]
    : [];
            ...
            <div className="sm:flex sm:items-start">
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 className="text-lg leading-6 font-medium text-gray-900">
                  {rooms.length > 1
                    ? `Are you sure you want to delete these ${rooms.length} rooms?`
                ...
                <div className="mt-2">
                 {rooms.length === 1 ? (
                    <>
                      ...
                      <p className="text-sm text-gray-500">
                        Room Number: {rooms[0].roomName}
                      </p>
                    </>
                  ) : (
                    <ul className="text-sm text-gray-500 max-h-32 overflow-y-auto">
                      {rooms.map((room) => (
                        <li key={room.id}>
                          {room.id} - {room.building} - {room.roomName}
                        </li>
                      ))}
                    </ul>
                  )}
